...
## CURRENT — Phase 3 (Desktop Client), IN PROGRESS. **CI red = GitHub Actions BILLING block (NOT code) — see ADR-0012.**
...
2. **Wave B2 was dispatched last session (nav + open-in-external + customizable shortcuts + settings, P08/P09) but did NOT land** (background agent was session-bound; only the lockfile is uncommitted). `git status` is clean except the lockfile. **Re-dispatch B2** after main is green (workspace switch/prev/next, new/quick-create, open project, open-in-external via the Electron main process cross-platform, customizable keyboard shortcuts + settings surface; on `@swarm/ui` + the REAL host).
3. Then: an **e2e CI job** (install Playwright chromium; run the desktop `test:e2e` — at least on ubuntu, ideally the matrix — so the real journey gates CI per §6.2), a **QA wave** (desktop + phone-width screenshots → `evidence/phase-3/`), then the **independent Phase-3 Critic INCLUDING the §6.3 anti-slop design bar** → `evidence/phase-3/review.md`. All-PASS → cut **v0.4.0**, move Linear P05/P06/P08/P09 to Done.
