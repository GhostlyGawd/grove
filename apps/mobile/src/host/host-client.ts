...
/** Query parameters for opening a terminal-IO WebSocket (P05 / W4). */
export interface TerminalParams {
  readonly workspaceId: string;
  /** Shell kind (`powershell` | `bash` …); host default when omitted. */
  readonly shell?: string;
  readonly cols: number;
  readonly rows: number;
  /** When set, the host runs this command non-interactively and streams it. */
  readonly cmd?: string;
}

/**
 * The ephemeral terminal-IO WS URL (separate topic from `/sync`). Like the sync
 * channel, the bearer rides the `token` query param because a browser WebSocket
 * cannot set an `Authorization` header; the host gates the upgrade on it. This is the
 * mobile mirror of the desktop `terminalUrl` — same wire contract, phone-supplied
 * geometry.
 */
